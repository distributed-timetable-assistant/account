...
/// Loads typed configuration according to the DiTA Service Configuration Guideline.
///
/// Precedence:
/// 1. Embedded static defaults (`config.yaml`)
/// 2. Configuration files (`./config.yml`, `./config-{APP_ENV}.yml`, `/etc/{app}/config.yml`, or `--config <path>`)
/// 3. Environment variables with prefix `APP_` and nested separator `__`
    ...
    C: DeserializeOwned + Debug,
{
    ...
    let config = conf_paths
        .iter()
        .map(|(path, required)| File::from(path.to_path_buf()).required(*required))
        .fold(
